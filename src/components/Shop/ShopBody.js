...
function ShopBody(props) {
    ...
    if (dateProductVirtual) {
        dateProductVirtual.sort((a,b)=> new Date(b.created) - new Date(a.created));
    ...
    const choosePrice = () => {
        const virtualProduct = []
        for (let i in constProduct) { 
            if (constProduct[i].current_price >= sortPriceValue[0] && constProduct[i].current_price <= sortPriceValue[1]) {
                virtualProduct.push(constProduct[i])
            }
        }
        setProduct(virtualProduct)
    ...
    const filterMobile = () => {
        ...
        if (cateFilter !== "") {
            for (let i in constProduct) { 
                if (constProduct[i].category_id == cateFilter) {
                    virtualProduct.push(constProduct[i])
                }
            }
        }
        ...
        for (let i in virtualProduct2) { 
            if (virtualProduct2[i].current_price >= sortPriceValue[0] && virtualProduct2[i].current_price <= sortPriceValue[1]) {
