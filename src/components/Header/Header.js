            ...
            .then(res => {
                ...
                for (let i in res.data) {
                    if (res.data[i].type == "Man") {
                        menProduct.push(res.data[i].category_id)
                    }
                    if (res.data[i].type == "Woman") {
                        womenProduct.push(res.data[i].category_id)
                    }
                }
                ...
                for (let i in groupCateMen) {
                    ...
                    for (let j in res.data) {
                        if (res.data[j].category_id == groupCateMen[i] && res.data[j].type == "Man") {
                            cateList.push(res.data[j].category_id)
                        }
                    }
                ...
                for (let i in groupCateWomen) {
                    ...
                    let cateList = []
                    for (let j in res.data) {
                        if (res.data[j].category_id == groupCateWomen[i] && res.data[j].type == "Woman") {
                            cateList.push(res.data[j].category_id)
                        }
                    }
                    let cateList2 = cateList.filter(function(elem, index, self) {
